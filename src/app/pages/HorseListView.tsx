    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 min-h-[400px]">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          ...
          <button 
            onClick={() => {
              setEditingHorse({ discipline: selectedCategory === 'sales' ? 'Sales' : (selectedCategory === 'dressage' ? 'Dressage' : 'Jumpers'), sex: 'Mare' }); setPendingMedia([]); setHorseMedia([]); setDeletedMediaIds([]); setShowHorseModal(true); 
            }}
            ...
            return (
              <div key={horse.id} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white hover:border-[#C2A878] hover:shadow-lg transition-all flex flex-col">
                ...
                <div 
                  ...
                  onClick={() => { setEditingHorse(horse); setPendingMedia([]); setHorseMedia([]); setDeletedMediaIds([]); fetchHorseMedia(horse.id); setShowHorseModal(true); }}
                >
                  <img 
