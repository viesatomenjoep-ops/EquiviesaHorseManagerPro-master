        ...
        <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
          <defs>
            <linearGradient id="activityGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#C2A878" stopOpacity={0.15} />
              <stop offset="100%" stopColor="#C2A878" stopOpacity={0.01} />
            </linearGradient>
          </defs>
          ...
          <path d={pathData} stroke="#C2A878" strokeWidth="2" fill="none" />

          {points.map((p, i) => (
            <circle key={`point-${i}`} cx={p.x} cy={p.y} r="3" fill="#C2A878" />
          ))}
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${device.status === 'active' ? 'bg-[#C2A878]' : 'bg-slate-300'}`}></div>
            ...
            <div className="flex items-center justify-between text-xs text-slate-500">
              ...
              <span className={`px-2 py-0.5 rounded-full font-medium ${
                device.status === 'active'
                  ? 'bg-amber-50 text-[#8C7345] border border-teal-100'
